...
class TargetsDAO(BaseDAO):
        ...
        def validator(dao_func: Callable):
            async def wrapper(*args: P.args, **kwargs: P.kwargs):
                ...
                if not RESULT_FROM_DB and critical:
                    ...
                    targets_dao_logger.warning("User requests object that is not exist or its already deleted")
    ...
    async def update_user_task(cls, task_id: int, raw_target: RawTarget) -> UpdatedTarget:
        """Update task selected by client and return task with updating fields"""

        async with cls.session_ as session:
            update_query = update(cls.current_model).where(Targets.id == task_id).values(
    ...
    async def get_by_target_id(cls, target_id: int) -> DetailedTarget:
        ...
        async with cls.session_ as session:
            ...
            #  Set Result as a  pydantic schemas
            adapter: DetailedTarget | None = TypeAdapter(DetailedTarget).validate_python(
                query_result.mappings().one_or_none()
            )
            return adapter
    ...
    async def find_all_by_id(cls, id_: int, limit: int = None) -> list[DetailedTarget]:
        ...
        async with cls.session_ as session:
            ...
            adapter = TypeAdapter(list[DetailedTarget]).validate_python(query_result.mappings().all())
            return adapter
