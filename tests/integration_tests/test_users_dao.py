...
class TestAuthenticatedUserTargetsDAOManipulations:
    ...
    async def test_new_task(
        self, test_database_preparation, authenticated_user_session
    ):
    ...
    async def test_my_tasks(self, authenticated_user_session):
        resp = await authenticated_user_session.get(url="/tasks")
        assert resp.status_code == 200
    ...
    async def test_remove_task(self, authenticated_user_session):
        task_id = await TargetsDAO.find_all_raw_task(
            get_fake_raw_target(
                title="test",
                description="test",
                status="done",
